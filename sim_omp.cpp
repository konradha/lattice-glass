...
int logand(uint8_t * x1, uint8_t * x2)
{
    ...
    {
        if (get_value(x1, i) == 1 && get_value(x2, i) == 1) nred++; 
        if (get_value(x1, i) == 2 && get_value(x2, i) == 2) nblue++;
    }
    return nred + nblue;
}
...
void build_lattice(const int &num_particles, const int &num_red,
  ...
  while(curr_red < num_red) {
    ...
    while(get_value_lattice(site, tid) != 0) site = indices(generator);
...
void concentrated_sweep(const float th, const float &beta, std::mt19937 &generator,
                    std::uniform_int_distribution<> &indices,
                    std::uniform_real_distribution<> &uni,
                    const int * nearest_neighbors, const int & tid)
{

    // need to reach certain threshold of low energy region -- could be parametrized, too
    if (energy(nearest_neighbors, tid) > (int)(.3333 * L * L * L)) {
        std::string s; s += "lattice " + std::to_string(tid) + " too hot, launching nonlocal sweep\n";
        nonlocal_sweep(L*L*L, beta, generator, indices, uni, nearest_neighbors, tid);
#pragma omp critical
        std::cout << s;
        return; 
    }
    ...
    for(int i=0;i<max_idx;++i)hottest_idx[i]=-1;
    ...
    for(int site=0;site<L*L*L;++site)
    {
        ...
        if (current_idx > max_idx - 1) break;
    }
                    ...
                    const int * nearest_neighbors, const int & tid)
{
    ...
    // can then get region with (std::get<0>(a), std::get<0>(a) + partition_size - 1)
    ...
    // a) highest <-> lowest
    const int coldest_region_start = std::get<0>(partition_energies[num_partitions - 1]); 
    ...
    */
    ...
    if (indices(generator) % 2 == 0) {
      for(int i=0;i<partition_size;++i) {
          const int site = warmest_region_start + (indices(generator) % (partition_size));
          const int mv   = coldest_region_start + (indices(generator) % (partition_size));
          if (get_value_lattice(site, tid) == get_value_lattice(mv, tid))
            continue; 
          const float E1 = nn_energy_packed(site, nearest_neighbors, tid) + nn_energy_packed(mv, nearest_neighbors, tid);
          exchange(site, mv, tid);
...
#pragma omp flush
  ...
  constexpr size_t max_collect = 100; // to have sufficient distance when copying in parallel
  constexpr size_t power2 = 12;
  uint8_t * config_collection = (uint8_t *) malloc(sizeof(uint8_t) * max_collect * NUM_THREADS * packed_size);
  ...
  // 1. partitioned "diffusion" sweeps -- build rotations: only two axes, ie. up and right
  // 2. check if e_avg <= \kappa \rho L³
  // 3. if reached: do concentrated sweeps with find max \kappa \rho L³ sites with local_e > 1
...
#pragma omp barrier
    ...
    for (int d = 1; d < 2 * TESTP + 1; ++d){ 
      ...
      {
        nonlocal_sweep(L * L * L, beta, generator, indices, uni, my_nn, tid); 
        concentrated_sweep(threshold, beta, generator, indices, uni, my_nn, tid);
      }
...
*/
  ...
  */
  ...
  short * serialized_configs = (short *) malloc(sizeof(short) * NUM_THREADS * cpycounter * L * L * L); 
  for(size_t t = 0; t < NUM_THREADS; ++t)
  {
      ...
      for(size_t d = 0; d < cpycounter; ++d)
      {
        ...
        uint8_t * current_config = config_collection + offset; 
        for(size_t s = 0; s < L*L*L; ++s)
            serialized_configs[(t * cpycounter * L * L * L) + (d * L * L * L) + s] = static_cast<short>(get_value(current_config, s));
      } 
  }
...
#ifdef DEBUG
  std::cout << "SAVING " << sizeof(short) * NUM_THREADS * cpycounter * L * L * L << " bytes\n"; 
#endif
