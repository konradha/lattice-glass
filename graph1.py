...
def build_graph3_pruned_cliques(k=6):
    g = nx.Graph()
    gs = []
    for _ in range(k):
...
def assign_color(g, rho):
    to_color_num = int(len(g.nodes()) * rho)
    return random.sample(list(g.nodes()), to_color_num)
...
def main():
    ...
    to_color_nodes = assign_color(g, .3)
    nx.draw(
        g,
        node_color=[
            "red" if node in to_color_nodes else "blue" for node in g.nodes()
        ],
    )
    plt.show()
